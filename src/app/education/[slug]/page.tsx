...
export async function generateStaticParams(): Promise<{ slug: string }[]> {
  const posts = getPosts(getEducationProjectsPath("es"));
...
export async function generateMetadata({
  ...
  const posts = getPosts(getEducationProjectsPath(locale));
  ...
  return Meta.generate({
    ...
    path: `${education.path}/${post.slug}`,
  });
}

export default async function EducationProject({
  params,
}: {
  params: Promise<{ slug: string | string[] }>;
}) {
  ...
  let post = getPosts(getEducationProjectsPath(locale)).find(
  ...
  return (
    <Column as="section" maxWidth="m" horizontal="center" gap="l">
      ...
      <Column maxWidth="s" gap="16" horizontal="center" align="center">
        <SmartLink href="/education">
          <Text variant="label-strong-m">{education.title}</Text>
        </SmartLink>
